...
function onConfirm() {
  const cb = ui.activeConfirmModal?.onConfirm
  ui.dismissConfirmModal()
  cb?.()
}

function onCancel() {
  const cb = ui.activeConfirmModal?.onCancel
  ui.dismissConfirmModal()
  cb?.()
}
</script>
  ...
  <Transition name="confirm">
    ...
    >
      ...
      <div class="relative w-full max-w-sm bg-white rounded-2xl shadow-xl p-6">
        <h3 class="text-sm font-semibold text-neutral-800">{{ ui.activeConfirmModal.title }}</h3>

        <p v-if="ui.activeConfirmModal.description" class="text-xs text-muted mt-2 leading-relaxed">
          {{ ui.activeConfirmModal.description }}
        </p>
        ...
        <div class="flex gap-3 mt-5">
          ...
          <button
            :class="ui.activeConfirmModal.variant === 'danger'
              ? 'border border-red-200 text-red-600 hover:bg-red-100 hover:border-red-300'
              : 'text-white hover:opacity-90'"
            :style="ui.activeConfirmModal.variant === 'danger'
              ? { background: '#fff5f5' }
