    ...
    >
      <div class="w-full max-w-md rounded-2xl bg-white shadow-xl p-6">
        ...
        <div class="rounded-lg bg-neutral-50 border border-neutral-200 p-3 mb-4 text-xs space-y-1">
          <div><span class="text-muted">Mod 名称：</span>{{ mods.pendingTrust.manifest.name }}</div>
          <div><span class="text-muted">ID：</span>{{ mods.pendingTrust.manifest.id }}</div>
          <div v-if="mods.pendingTrust.manifest.author">
            <span class="text-muted">作者：</span>{{ mods.pendingTrust.manifest.author }}
          </div>
          <div><span class="text-muted">来源：</span>{{ mods.pendingTrust.source }}</div>
        </div>
        <div class="flex gap-2">
          <button
            class="flex-1 text-sm py-2 rounded-xl border border-neutral-200 hover:border-neutral-400 transition-colors"
            @click="mods.cancelTrust()"
          >
            取消
          </button>
          <button
            ...
            @click="mods.trustAndEnable(mods.pendingTrust!.manifest)"
          >
            安装
          </button>
        </div>
